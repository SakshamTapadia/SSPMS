...
/// <summary>
/// Sends transactional email via the Brevo HTTP API (brevo.com).
/// Set Brevo__ApiKey in environment variables to activate.
/// Free tier: 300 emails/day, 9 000/month — no custom domain required,
/// just verify your sender address in the Brevo dashboard.
/// </summary>
public class BrevoEmailService : IEmailService
{
    ...
    public BrevoEmailService(IHttpClientFactory httpFactory, IConfiguration config, ILogger<BrevoEmailService> logger)
    {
    ...
    public async Task SendAsync(string to, string subject, string htmlBody)
    {
        ...
        var payload = new
        {
            sender      = new { name = display, email = fromAddr },
            to          = new[] { new { email = to } },
            subject     = subject,
        ...
        _logger.LogInformation("Email sent via Brevo to {To} — subject: {Subject}", to, subject);
    }
